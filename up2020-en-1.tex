...
first time-sharing operating system in history, was quite successful, and its success
resulted in a much more ambitious project called Multics\cupercite{wiki:multics}
(with development starting in 1964).  The project did not deliver a commercially usable
...
home router now; in comparison, modern Linux systems can only run on the
...
reasonable, and the script from last section is easy to debug and
...
service directory to run the corresponding system service.  The user can use
s6's command line tools \verb|s6-svc|/\verb|s6-svscanctl| to interact
with \verb|s6-supervise|/\verb|s6-svscan|, and can use ancillary files
in service directories and the scan directory to modify the behaviours of
...
the first point by McIlroy is an emphasis on high cohesion and low
...
collaboration and reuse of command line programs\cupercite{salus1994}; since
McIlroy is the inventor of Unix pipes, his summary was probably based on
...
some relevant cases outside shell scripting that reflect the
...
state control needs to be in init, or in other words what systemd does is
...
below; \cf~\parencite{pollard2014} for some more examples), which sets its own
...
to \verb|/dev/log| which is listened on by a system logger; so all
...
by malicious actors, then security vulnerabilities, like other defects, are usually
...
mental labour, and the same kind of products by the same person that consumed
the same amount of energy should naturally contain similar numbers of defects.
Defects (also including security vulnerabilities) in
software systems come with code changes, and go with analyses and debugging, while
...
we also need actual methods to improve our abilities to write simple code;
