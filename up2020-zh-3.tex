...
Scheme 没有 \texttt{return} 原语，而是使用\stress{延续}\cupercite{wiki:cont}%
以及\stress{尾位置}表达式来返回值；第二，在 Scheme 中 \texttt{(define (func
...
为了理解 S 表达式对机器的有好性，我们首先须要了解程序源代码在计算机内部
...
libtask\cupercite{swtch:libtask} 模式轻量级协程\footnote{另一种模式是
基于\stress{continuation-passing style}\cupercite{wiki:cps}（CPS）的。}%
所提供接口的\stress{语法糖}，它们被硬编码在 Go 中的原因正是 Go 不能
...
{顺便提到，这令人联想到 CPS，而这种关联并非巧合\cupercite{troelskn2009}。}
\begin{quoting}
\begin{Verbatim}
f5 = lambda x, y: check(
  x, (lambda x: check(
...
使用这些\stress{结构宏}来紧凑地编写要在被处理的源代码上进行的变换，而且可以将
这些代码变换实现为多个简洁清晰且可靠的步骤，后者就像通过管道连接的传统 Unix
工具一样。这种\stress{多步处理}方案的最佳实例\footnote{\label{fn:slew}注意到第
...
设计。}是 nanopass 框架\cupercite{andersen2016}\footnote{顺便提到，nanopass
主要作者之一的姓名首字母简写是“AWK”。}，它被久负盛名的 Scheme 编译器 Chez
Scheme\cupercite{wiki:chez} 使用，后者在 2016 年被开源；Chez Scheme
...
{chiusano2014}。}，Lisp 会选择前者（“\stress{the right thing}”，或 MIT/Stanford
作风），而 Unix 会选择后者（“\stress{worse is better}”，或 New Jersey 作风）。
...
语言中实现\cupercite{vector2016a}（参考如 \verb|cd| 和 \verb|umask| 等等 shell
...
无疑地认为\stress{通过应用同像性，我们将能构造比超越从前可能的 Unix 式系统}。
...
在本节中，我们至今考虑的是 Nil 的类似于 Lisp 的方面，那么它类似于 C 的方面是怎样
...
\stress{两种语言最佳协作方式}（最小化系统复杂度、最大化程序员效率）的探索将必然
